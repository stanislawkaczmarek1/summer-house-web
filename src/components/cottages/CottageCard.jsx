  ...
  return (
    <article className="cottage-card">
      <Link to={`/cottages/${id}`} className="cottage-card__img-wrap">
        {coverImage ? (
          <img src={coverImage} alt={name} className="cottage-card__img" loading="lazy" />
        ) : (
          <div className="cottage-card__img-placeholder"><span>⌂</span></div>
        )}
        {!visible && <div className="cottage-card__hidden-badge">Hidden</div>}
        <div className="cottage-card__img-overlay" />
      </Link>

      <div className="cottage-card__body">
        <div className="cottage-card__meta">
          <span className="cottage-card__capacity">◎ Up to {capacity} guests</span>
        </div>
        ...
        {address && <p className="cottage-card__address">⊹ {address}</p>}
        {description && <p className="cottage-card__desc">{description}</p>}
